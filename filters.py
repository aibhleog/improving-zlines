...
jwst_miri_spec = {'lrs': miri_lrs, 'mrs': miri_mrs}
jwst_niriss_spec = {'wfss': niriss_wfss, 'soss': niriss_soss}
jwst_nircam_spec = {'wfss': nircam_wfss}
jwst_nirspec = {'prism': nirspec_prism, 'f070lp': nirspec_mrs1, 
                'f100lp': nirspec_mrs2, 'f170lp': nirspec_mrs3, 'f290lp': nirspec_mrs4}


# JWST/MIRI, JWST/NIRISS, JWST/NIRCam 
# ----- PHOTOMETRY bands
...
jwst_nircam_phot, jwst_niriss_phot, jwst_miri_phot = {},{},{}

for filt in nircam_filters: 
    jwst_nircam_phot[filt] = pd.read_csv(path+f'JWST_NIRCam.{filt.upper()}.dat',
                                    names=cols,sep='\s+')

for filt in niriss_filters: 
    jwst_niriss_phot[filt] = pd.read_csv(path+f'JWST_NIRISS.{filt.upper()}.dat',
                                    names=cols,sep='\s+')

for filt in miri_filters: 
    jwst_miri_phot[filt] = pd.read_csv(path+f'JWST_MIRI.{filt.upper()}.dat',
                                  names=cols,sep='\s+')
...
jwst_filters_spec = {'jwst/nirspec': jwst_nirspec, 'jwst/niriss': jwst_niriss_spec, 
                     'jwst/nircam': jwst_nircam_spec, 'jwst/miri': jwst_miri_spec}
jwst_filters_phot = {'jwst/niriss': jwst_niriss_phot, 'jwst/nircam': jwst_nircam_phot, 
                     'jwst/miri': jwst_miri_phot}
...
def get_filter(telinst,band,jwst=False,phot=False):
    # added a jwst tag because those are plotted differently
    if jwst == True: 
        if phot == False: filters_list = jwst_filters_spec
        else: filters_list = jwst_filters_phot
...
def is_it_in_range(z,jwst=False,phot=False):
...
# JWST/MIRI, JWST/NIRISS, JWST/NIRCam
# ----- PHOTOMETRY bands
jwst_nircam_phot_color, jwst_niriss_phot_color, jwst_miri_phot_color = {},{},{}

filler_colors = [plt.get_cmap('cmr.lavender_r')(j) for j in np.linspace(0.14,0.8,len(nircam_filters))]
for filt in nircam_filters: 
    indx = nircam_filters.index(filt)
    jwst_nircam_phot_color[filt] = {'name':filt.upper(), 'color':filler_colors[indx],
                                    'edgecolor':filler_colors[indx]}

filler_colors = [plt.get_cmap('cmr.guppy_r')(j) for j in np.linspace(0.14,0.6,len(niriss_filters))]
for filt in niriss_filters: 
    indx = niriss_filters.index(filt)
    jwst_niriss_phot_color[filt] = {'name':filt.upper(), 'color':filler_colors[indx],
                                    'edgecolor':filler_colors[indx]}

filler_colors = [plt.get_cmap('cmr.ember_r')(j) for j in np.linspace(0.3,0.8,len(miri_filters))]
for filt in miri_filters: 
    indx = miri_filters.index(filt)
    jwst_miri_phot_color[filt] = {'name':filt.upper(), 'color':filler_colors[indx],
                                  'edgecolor':filler_colors[indx]}
...
# jwst filters, to be added differently
jwst_plotting_spec = {'jwst/nirspec': jwst_nirspec, 'jwst/niriss': jwst_niriss_spec, 
                      'jwst/nircam': jwst_nircam_spec, 'jwst/miri': jwst_miri_spec}
jwst_plotting_phot = {'jwst/niriss': jwst_niriss_phot_color, 
                      'jwst/nircam': jwst_nircam_phot_color, 
                      'jwst/miri': jwst_miri_phot_color}


def get_filter_info(telinst,band,jwst=False,phot=False):
    # added a jwst tag because those are plotted differently
    if jwst == True: 
        if phot == False: plotting_list = jwst_plotting_spec
        else: plotting_list = jwst_plotting_phot
